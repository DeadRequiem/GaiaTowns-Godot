...
func _process(delta: float) -> void:
	...
	if attempting_connection:
		...
		if connection_timer >= Config.CONNECTION_TIMEOUT:
...
func attempt_reconnect() -> void:
	if reconnect_attempts < Config.MAX_RECONNECT_ATTEMPTS:
		...
		print("NetworkManager: Reconnect attempt %d/%d" % [reconnect_attempts, Config.MAX_RECONNECT_ATTEMPTS])
		
		await get_tree().create_timer(Config.RECONNECT_DELAY * reconnect_attempts).timeout
		connect_to_server()
	else:
