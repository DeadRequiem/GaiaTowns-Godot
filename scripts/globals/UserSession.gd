...
func get_random_fallback_avatar() -> String:
	"""Pick a random fallback avatar from the folder"""
...
func load_credentials() -> void:
	"""Load username, avatar URL, and admin token from the web page"""
...
func is_valid_avatar_url(url: String) -> bool:
	"""Validate that avatar URL is from an allowed domain"""
	if url.is_empty():
		return false
...
func get_proxied_avatar_url(url: String) -> String:
	"""Convert an avatar URL to use the proxy with cache busting"""
	if url.begins_with("res://"):
		return url
	...
	var url_with_cache_buster = url + separator + "t=" + str(timestamp)
