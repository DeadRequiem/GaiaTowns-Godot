...
func load_credentials_from_web():
	"""Load username, avatar URL, and admin token from the web page JavaScript"""
	if OS.has_feature("web"):
		...
		var admin_token_js = JavaScriptBridge.eval("document.cookie.split('; ').find(row => row.startsWith('admin_token='))?.split('=')[1] || ''")
...
func join_barton(username: String, avatar_url: String, barton_id: int, map_id: int, position: Vector2):
	if not connected:
		...
		return
	# Send admin token to server for verification
	_join_barton.rpc_id(1, username, avatar_url, barton_id, map_id, position, admin_token)
...
@rpc("authority", "reliable")
func _confirm_admin_status(confirmed: bool):
	is_admin = confirmed
	admin_status_confirmed.emit(confirmed)
	if confirmed:
		print("[CLIENT] Admin status confirmed by server")
	else:
		print("[CLIENT] Not an admin")
