...
func return_bubble_to_pool(bubble: Node) -> void:
	...
	# Only keep bubbles in pool if under max size
	if bubble_pool.size() < MAX_POOL_SIZE:
		...
		# Pool full, just free it to avoid memory bloat
		bubble.queue_free()
...
func update_stack_position(player_id: int) -> void:
	...
	var screen_pos := world_pos - camera.get_screen_center_position() + get_viewport().get_visible_rect().size / 2
	
	var stack_height: float = stack.size.y
	screen_pos.y -= 80
	screen_pos.y -= stack_height
	screen_pos.x -= 67.5
...
func update_all_positions() -> void:
	var player_ids := player_stacks.keys()
