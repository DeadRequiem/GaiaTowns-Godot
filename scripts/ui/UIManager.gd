...
func _handle_paste_with_retry() -> void:
	...
	if new_text.length() > Config.MAX_MESSAGE_LENGTH:
		new_text = new_text.substr(0, Config.MAX_MESSAGE_LENGTHMAX_MESSAGE_LENGTH)

	chat_input.text = new_text

	var new_caret_pos = min(insert_pos + clipboard_text.length(), Config.MAX_MESSAGE_LENGTHH)
...
func _on_chat_text_changed() -> void:
	if chat_input.text.length() > Config.MAX_MESSAGE_LENGTH:
		var caret_line = chat_input.get_caret_line()
		var caret_col = chat_input.get_caret_column()
		chat_input.text = chat_input.text.substr(0, Config.MAX_MESSAGE_LENGTH)
		chat_input.set_caret_line(caret_line)
		chat_input.set_caret_column(min(caret_col, Config.MAX_MESSAGE_LENGTH))
...
func add_chat_message(username: String, message: String) -> void:
	...
	if message_count > Config.MAX_CHAT_LOG_MESSAGES:
		...
		if first_newline != -1:
			...
			message_count = Config.MAX_CHAT_LOG_MESSAGES
