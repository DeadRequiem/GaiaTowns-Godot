...
func add_message(text: String) -> void:
	var lifetime = Config.get_bubble_lifetime(text)
	...
	if messages.size() > Config.MAX_CHAT_BUBBLE_MESSAGES:
		messages.pop_front()
...
func _rebuild_bubble() -> void:
	...
	drawer.position = Vector2(-Config.BUBBLE_WIDTH / 2.0, -total_height)
...
class BubbleDrawer extends Node2D:
	...
	func _calculate_dimensions() -> void:
		...
		if not username.is_empty():
			total_height += Config.USERNAME_FONT_SIZE * Config.LINE_HEIGHT_MULTIPLIER + Config.USERNAME_PADDING * 2
			total_height += 1
		
		for msg in messages:
			var lines = _wrap_text(msg.text, Config.BUBBLE_WIDTH - (Config.MESSAGE_PADDING * 2), Config.MESSAGE_FONT_SIZE)
			total_height += (lines.size() * Config.MESSAGE_FONT_SIZE * Config.LINE_HEIGHT_MULTIPLIER) + (Config.MESSAGE_PADDING * 2) + Config.MESSAGE_SPACING
	...
	func _draw() -> void:
		...
		if not username.is_empty():
			var username_height = Config.USERNAME_FONT_SIZE * Config.LINE_HEIGHT_MULTIPLIER + Config.USERNAME_PADDING * 2
			var username_rect = Rect2(0, y_offset, Config.BUBBLE_WIDTH, username_height)
			
			_draw_rounded_rect(username_rect, Config.USERNAME_BG_COLOR, Config.MESSAGE_BORDER_COLOR, true, true, false, false)
			
			var text_x = Config.MESSAGE_PADDING
			var text_y = y_offset + Config.USERNAME_PADDING + Config.USERNAME_FONT_SIZE
			draw_string(chat_font, Vector2(text_x, text_y), username, HORIZONTAL_ALIGNMENT_LEFT, -1, Config.USERNAME_FONT_SIZE, Config.MESSAGE_TEXT_COLOR)
			
			y_offset += username_height
			
			if messages.size() > 0:
				draw_line(Vector2(2, y_offset), Vector2(Config.BUBBLE_WIDTH - 2, y_offset), Config.DIVIDER_COLOR, 1)
				y_offset += 1
		
		for i in messages.size():
			var msg = messages[i]
			var lines = _wrap_text(msg.text, Config.BUBBLE_WIDTH - (Config.MESSAGE_PADDING * 2), Config.MESSAGE_FONT_SIZE)
			var message_height = (lines.size() * Config.MESSAGE_FONT_SIZE * Config.LINE_HEIGHT_MULTIPLIER) + (Config.MESSAGE_PADDING * 2)
			
			var message_rect = Rect2(0, y_offset, Config.BUBBLE_WIDTH, message_height)
			
			var is_first = (i == 0 and username.is_empty())
			var is_last = (i == messages.size() - 1)
			
			_draw_rounded_rect(message_rect, Config.MESSAGE_BG_COLOR, Config.MESSAGE_BORDER_COLOR, is_first, is_first, is_last, is_last)
			
			var text_y = y_offset + Config.MESSAGE_PADDING + Config.MESSAGE_FONT_SIZE
			for line in lines:
				var text_x = Config.MESSAGE_PADDING
				draw_string(chat_font, Vector2(text_x, text_y), line, HORIZONTAL_ALIGNMENT_LEFT, -1, Config.MESSAGE_FONT_SIZE, Config.MESSAGE_TEXT_COLOR)
				text_y += Config.MESSAGE_FONT_SIZE * Config.LINE_HEIGHT_MULTIPLIER
			
			y_offset += message_height + Config.MESSAGE_SPACING
