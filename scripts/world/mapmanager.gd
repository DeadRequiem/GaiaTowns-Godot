...
func transition_to_map(player_body: Node2D, new_row: int, new_col: int, came_from: String) -> void:
	...
	await load_map(next_map_id, spawn_pos)
...
func calculate_spawn_position(came_from: String) -> Vector2:
	match came_from:
		"north":
			return Vector2(map_width / 2, Config.EDGE_CLEARANCE)
		"south":
			return Vector2(map_width / 2, map_height - Config.EDGE_CLEARANCE)
		"west":
			return Vector2(Config.EDGE_CLEARANCE, map_height / 2)
		"east":
			return Vector2(map_width - Config.EDGE_CLEARANCE, map_height / 2)
		_:
